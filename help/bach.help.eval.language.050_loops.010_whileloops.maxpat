...
, 			{
				"box" : 				{
					"code" : "$i := 1 ; while $i < $l1 repeat print($i := $i + 1) ",
...
,
					"text" : "bach.eval $i := 1 \\; while $i < $l1 repeat print($i := $i + 1) @out m"
				}
...
, 			{
				"box" : 				{
					...
					"text" : "The syntax of the \"while\" loop is:\nwhile <condition> repeat <sub-expression>\nThe condition is an expression returning true or false according to the same criteria as \"if...then\".\nThe \"while\" loop first evaluates condition; if it is true, it evaluates the sub-expression and re-evaluates the condition; if it is still true, it evaluates the sub-expression again, and so on until the condition remains true. As soon as the condition is false, the iteration stops, returning the value of the sub-expression at the end of the last iteration.\nIf the condition is false at the beginning of the first iteration, the sub-expression doesn't get evaluated at all and the loop returns null.\nIn general, care must be taken for the condition to actually change its truth value at some point, otherwise the evaluation gets stuck in an infinite loop. Anyway, bach.eval has a maxtime attribute setting the maximum duration of an evaluation, so you won't be forced to quit Max anyway.\nBecause bach.eval is not interactive, that is, you can't feed data into it while its computation is running, there is no way to change the truth value from the outside, and you need at least one control variable involved, keeping track of some data to watch at every iteration of the loop. If you are a fan of functional programming languages, worry not: bach.eval has map, reduce and recursive functions, so you'll never be forced to use loops if you don't like them.",
