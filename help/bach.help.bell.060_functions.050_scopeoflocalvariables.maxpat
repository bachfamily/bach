{
    "patcher": {
        "fileversion": 1,
        "appversion": {
            "major": 9,
            "minor": 1,
            "revision": 1,
            "architecture": "x64",
            "modernui": 1
        },
        "classnamespace": "box",
        "rect": [ 356.0, 171.0, 617.0, 539.0 ],
        "default_fontsize": 13.0,
        "boxes": [
            {
                "box": {
                    "id": "obj-6",
                    "maxclass": "button",
                    "numinlets": 1,
                    "numoutlets": 1,
                    "outlettype": [ "bang" ],
                    "parameter_enable": 0,
                    "patching_rect": [ 18.0, 331.0, 24.0, 24.0 ]
                }
            },
            {
                "box": {
                    "id": "obj-7",
                    "maxclass": "newobj",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 18.0, 405.0, 98.0, 23.0 ],
                    "text": "print @popup 1"
                }
            },
            {
                "box": {
                    "code": "$f = $x -> (print($var); $var = $x); for $i in 1...5 collect $f($i) ",
                    "id": "obj-8",
                    "linecount": 2,
                    "maxclass": "newobj",
                    "numinlets": 1,
                    "numoutlets": 1,
                    "outlettype": [ "" ],
                    "patching_rect": [ 18.0, 363.0, 262.0, 38.0 ],
                    "saved_object_attributes": {
                        "embed": 1,
                        "versionnumber": 80007
                    },
                    "text": "bach.eval $f = $x -> (print($var)\\; $var = $x)\\; for $i in 1...5 collect $f($i) @out m"
                }
            },
            {
                "box": {
                    "bubble": 1,
                    "fontname": "Arial",
                    "fontsize": 13.0,
                    "id": "obj-9",
                    "linecount": 2,
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 291.0, 362.0, 239.0, 40.0 ],
                    "text": "At the beginning of each evaluation, the value of $var is reset to null."
                }
            },
            {
                "box": {
                    "fontname": "Arial",
                    "fontsize": 13.0,
                    "id": "obj-3",
                    "linecount": 3,
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 6.0, 254.0, 563.0, 50.0 ],
                    "text": "It is important to observe that the value of all local variables is reset at every call of the function. By \"reset\" we usually mean that each variable is set to null, but it is also possible to set defaults. There is a way to make the value of a variable persistent, as we shall discuss shortly.",
                    "textcolor": [ 0.5, 0.5, 0.5, 1.0 ]
                }
            },
            {
                "box": {
                    "id": "obj-5",
                    "maxclass": "button",
                    "numinlets": 1,
                    "numoutlets": 1,
                    "outlettype": [ "bang" ],
                    "parameter_enable": 0,
                    "patching_rect": [ 18.0, 136.0, 24.0, 24.0 ]
                }
            },
            {
                "box": {
                    "id": "obj-2",
                    "maxclass": "newobj",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 18.0, 210.0, 98.0, 23.0 ],
                    "text": "print @popup 1"
                }
            },
            {
                "box": {
                    "code": "$i = 1 ; $f = ($n -> for $i in 1 to $n do print($i)) ; $f(5) $i ",
                    "id": "obj-1",
                    "linecount": 2,
                    "maxclass": "newobj",
                    "numinlets": 1,
                    "numoutlets": 1,
                    "outlettype": [ "" ],
                    "patching_rect": [ 18.0, 168.0, 231.0, 38.0 ],
                    "saved_object_attributes": {
                        "embed": 1,
                        "versionnumber": 80005
                    },
                    "text": "bach.eval $i = 1 \\; $f = ($n -> for $i in 1 to $n do print($i)) \\; $f(5) $i @out m"
                }
            },
            {
                "box": {
                    "bubble": 1,
                    "fontname": "Arial",
                    "fontsize": 13.0,
                    "id": "obj-18",
                    "linecount": 3,
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 257.0, 160.0, 273.0, 54.0 ],
                    "text": "The fact that we use and modify a variable called $i in the $f function doesn't affect the $i variable in the main function."
                }
            },
            {
                "box": {
                    "fontname": "Arial",
                    "fontsize": 13.0,
                    "id": "obj-4",
                    "linecount": 5,
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 6.0, 36.0, 585.0, 79.0 ],
                    "text": "Saying that the visibility of local variables is limited to the object is inaccurate when functions come into play. In fact, by default local variables are only visible within the function in which they are used.\nTo really make the concept solid, let's define the main function of an object as the portion of code that is not contained in any function definition (because, as a matter of fact, it is a function, whose arguments can be set from the outside).",
                    "textcolor": [ 0.5, 0.5, 0.5, 1.0 ]
                }
            },
            {
                "box": {
                    "fontname": "Arial",
                    "fontsize": 11.0,
                    "id": "obj-11",
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 6.0, 514.0, 585.0, 19.0 ],
                    "text": "See Also: ",
                    "textcolor": [ 0.5, 0.5, 0.5, 1.0 ],
                    "varname": "seealso"
                }
            },
            {
                "box": {
                    "fontname": "Arial",
                    "fontsize": 11.0,
                    "id": "obj-10",
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 6.0, 495.0, 585.0, 19.0 ],
                    "text": "Objects: bach.eval, bach.diff, bach.intersection, bach.symdiff, bach.union",
                    "textcolor": [ 0.5, 0.5, 0.5, 1.0 ],
                    "varname": "objects"
                }
            },
            {
                "box": {
                    "fontface": 1,
                    "fontname": "Arial",
                    "fontsize": 15.0,
                    "id": "obj-13",
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 6.0, 8.0, 179.0, 23.0 ],
                    "text": "Scope of local variables",
                    "varname": "title"
                }
            },
            {
                "box": {
                    "fontname": "Arial",
                    "fontsize": 11.0,
                    "id": "obj-12",
                    "linecount": 2,
                    "maxclass": "comment",
                    "numinlets": 1,
                    "numoutlets": 0,
                    "patching_rect": [ 6.0, 476.0, 585.0, 31.0 ],
                    "text": "Tags: evaluation, language, script, scripting, code, bell, function, user, defined, variable, local, assignment, scope, visibility, dynamic, scoping, propagation",
                    "textcolor": [ 0.5, 0.5, 0.5, 1.0 ],
                    "varname": "tags"
                }
            }
        ],
        "lines": [
            {
                "patchline": {
                    "destination": [ "obj-2", 0 ],
                    "source": [ "obj-1", 0 ]
                }
            },
            {
                "patchline": {
                    "destination": [ "obj-1", 0 ],
                    "source": [ "obj-5", 0 ]
                }
            },
            {
                "patchline": {
                    "destination": [ "obj-8", 0 ],
                    "source": [ "obj-6", 0 ]
                }
            },
            {
                "patchline": {
                    "destination": [ "obj-7", 0 ],
                    "source": [ "obj-8", 0 ]
                }
            }
        ],
        "autosave": 0
    }
}