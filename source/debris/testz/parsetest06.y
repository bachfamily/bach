...
| lvalue NTH VAR {
    printf("   : %d ", vartable[$3].v[0]);
    if ($1->pos < 0)
        $1->pos = (vartable[$3].v[0]) - 1;
    else
        $1->pos += vartable[$3].v[0];
    printf("  pos changed to %d\n", $1->pos);
    $$ = $1;
}
;
...
chain: chainBricks %prec CHAIN
| chainBricks assignment %prec CHAIN {
    $$ = intarray_chain($1, $2);
}
;
