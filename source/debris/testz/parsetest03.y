...
%type <a> sequence_end
%type <a> chainChain
%type <r> lvalue fakelvalue lvalueChain fakelvalueChain
...
fakelvalue: item NTH value {
    printf("item NTH value\n  ");
    intarray_print($1, 1);
    $$ = reference_new($1, ($3->v[0]) - 1);
    printf("  pos changed to %d\n", $$->pos);
}
...
| fakelvalue ASSIGN chain { // fakevalue surely has a position
    int pos = $1->pos;
    $$ = intarray_copy($1->a, NULL);
    $$->v[pos] = ($3->v)[0];
    if ($$->n <= pos) {
...
chain: chainChain
| chainChain assignment {
    $$ = intarray_chain($1, $2);
}
| chainChain conditional {
  $$ = intarray_chain($1, $2);
}
;
