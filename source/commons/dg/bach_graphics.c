...
// NONE OF THIS BELOW IS WORKING PROPERLY; this was meant to solve a bug by cycling
double bach_get_patching_height(t_object *ob)
{
    t_size rect;
    jbox_get_patching_size(ob, &rect);
    object_post(ob, "size: %.2f", rect.height);
    return rect.height;
}

void bach_init_size_do(t_object *ob, long init_width, long init_height)
{
    t_size pres_rect, patch_rect;
    jbox_get_presentation_size(ob, &pres_rect);
    jbox_get_patching_size(ob, &patch_rect);
    
    pres_rect.height = patch_rect.height = init_height;
    pres_rect.width = patch_rect.width = init_width;
    jbox_set_presentation_size(ob, &pres_rect);
    jbox_set_patching_size(ob, &patch_rect);
}

void bach_init_size_gimme(t_object *ob, t_symbol *msg, long ac, t_atom *av)
{
    if (ac >= 2)
        bach_init_size_do(ob, atom_getlong(av), atom_getlong(av+1));
}
...
{
//    bach_init_size_do(ob, init_width, init_height);
    
/*    t_atom av[2];
    atom_setlong(av, init_width);
    atom_setlong(av+1, init_height);
    defer(ob, (method)bach_init_size_gimme, NULL, 2, av); */
}
