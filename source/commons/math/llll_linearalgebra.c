	...
	for (row_elem1 = matrix1->l_head, row_elem2 = matrix2->l_head; row_elem1 && row_elem2; row_elem1 = row_elem1->l_next, row_elem2 = row_elem2->l_next) {
		if (hatom_gettype(&row_elem1->l_hatom) == H_LLLL && hatom_gettype(&row_elem2->l_hatom) == H_LLLL) {
			...
			for (column_elem1 = matrix1->l_head, column_elem2 = matrix2->l_head; column_elem1 && column_elem2; 
				...
				t_hatom res;
				err = lexpr_eval_one(op, &column_elem1->l_hatom, &column_elem2->l_hatom, NULL, NULL, &res);
				if (!err)
			...
			t_hatom res;
			err = lexpr_eval_one(op, &row_elem1->l_hatom, &row_elem2->l_hatom, NULL, NULL, &res);
			if (!err)
...
t_hatom llll_vector_vector_product(t_llll *vector1, t_llll *vector2){
	...
	for (elem1 = vector1->l_head, elem2 = vector2->l_head; elem1 && elem2; elem1 = elem1->l_next, elem2 = elem2->l_next) {
		...
		err = lexpr_eval_one(&times, &elem1->l_hatom, &elem2->l_hatom, NULL, NULL, &partial_res_hatom);
		if (!err)
			err = lexpr_eval_one(&plus, &res_hatom, &partial_res_hatom, NULL, NULL, &res_hatom);
	}
	return res_hatom;
}
	...
	while (1) {
		while (inelem) {
			switch(inelem->l_hatom.h_type) {
				...
				case H_RAT:
				case H_DOUBLE:
					err = lexpr_eval_one(&times, &inelem->l_hatom, scalar, NULL, NULL, &partial_res_hatom);
					if (!err)
...
t_llll* llll_matrix_matrix_product(t_llll *llll1, t_llll *llll2){
	...
	if (llll1->l_size == 1 && llll1->l_depth == 1 && llll2->l_size == 1 && llll2->l_depth == 1) { 
		...
		out_llll = llll_get();
		err = lexpr_eval_one(&times, &llll1->l_head->l_hatom, &llll2->l_head->l_hatom, NULL, NULL, &res_hatom);
		if (err)
			return NULL;
...
t_hatom llll_trace(t_llll *matrix, long shift){
	...
	for (i = 0, elem1 = matrix->l_head; elem1; i++, elem1 = elem1->l_next) {
		if (hatom_gettype(&elem1->l_hatom) == H_LLLL) {
			...
			if (elem2)
				err = lexpr_eval_one(&plus, &res_hatom, &elem2->l_hatom, NULL, NULL, &res_hatom);
		}
	}
	return res_hatom;
}
...
//	char *debug;
	...
	while (i <= num_rows && j <= num_cols) {
		...
		if (det && !llll_eq_hatom_ignoretype(det, &zero)) 
			lexpr_eval_one(&times, det, &pivot_hatom, NULL, NULL, det);
		...
		if (llll_gt_hatom(&pivot_abs_hatom, &zero)) {
			...
			for (k = i+1; k <= num_rows; k++) {
				...
				for (h = 1; h <= num_cols; h++) {
					...
					err = lexpr_eval_one(&times, &multiplier, &el_to_subtract->l_hatom, NULL, NULL, &to_sum);
					err = lexpr_eval_one(&times, &to_sum, &minusone, NULL, NULL, &to_sum);
					err = lexpr_eval_one(&plus, &el_to_operate_on->l_hatom, &to_sum, NULL, NULL,  &el_to_operate_on->l_hatom);
				}
	...
	if (det && (swaps % 2 != 0))
		lexpr_eval_one(&times, det, &minusone, NULL, NULL, det);
...
t_hatom llll_get_cofactor(t_llll *matrix, long row, long col) {
	...
	if ((row + col) % 2 == 1)
		lexpr_eval_one(&times, &cofactor, &minus_one, NULL, NULL, &cofactor);
...
t_llll* llll_proj(t_llll *vector, t_llll *direction) { 
	...
	t_hatom multiplier;
	lexpr_eval_one(&div, &num, &den, NULL, NULL, &multiplier);
