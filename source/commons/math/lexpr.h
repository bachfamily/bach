...
typedef struct _lexpr_func {
    ...
    union {        // of course, the fields of the union are all the same. this is only useful to avoid some casting
        ...
        double        (*p_dptr_dddd)(double a, double b, double c, double d);    // the function, if it will return a double (e.g. cos) - use it with f_type H_DOUBLE
        ...
        void        (*p_vptr_hhhhh)(t_hatom *a, t_hatom *b, t_hatom *c, t_hatom *d, t_hatom *r);    // use this with H_ALL. the function returns void, but accepts a last *hatom parameter for storing the result (see lexpr_float or lexpr_pow)
    } f_ptrs;
} t_lexpr_func;
