...
#ifndef lint
#ifdef MAC_VERSION
    static char __unused identstring[] = "$File: " __FILE__ " $  $SDIFVersion: " SDIF_VERSION_STRING " $ IRCAM $SDIFcompiled: " __DATE__ " $";
#endif
#ifdef WIN_VERSION
  ...
  if (SdifF)
    {
	 ...
	 eBinaryModeUnknown means file i/o */
      ...
      if (SdifF->Name [0] == 0  ||  SdifStrEq (SdifF->Name, "-"))
		  stdio = Mode == eReadFile	?  eBinaryModeStdInput	:  eBinaryModeStdOutput;
      else
...
{
  SdiffPosT pos = 0;
  SdiffGetPosNoCond(file->Stream, &pos);
