...
char* get_string_for_tick(t_hatom *h, long maxdecimals)
{
    if (hatom_gettype(h) == H_OBJ) {
        ...
        snprintf_zero(out, 20, "(%s%ld%s%s)",
                      type == k_WHOLE_NOTATION_OBJECT ? "w" : (type == k_HEADER_DATA ? "h" : (type == k_MEASURE ? "m" : (type == k_CHORD ? "c" : (type == k_VOICE ? "v" : (type == k_NOTE ? "n" : (type == k_MARKER ? "k" : "?")))))),
                      ID,
                      modif == k_UNDO_MODIFICATION_TYPE_INSERT ? "+" : (modif == k_UNDO_MODIFICATION_TYPE_REMOVE ? "-" : (modif == k_UNDO_MODIFICATION_TYPE_CHANGE ? "~" : (modif == k_UNDO_MODIFICATION_TYPE_CHANGE_CHECK_ORDER ? "*" : "?"))),
...
t_llllelem *undo_redo_step_marker_create(t_notation_obj *r_ob, char what, char from_what, long undo_op, char also_lock_undo_mutex)
{
    ...
    if (what == k_UNDO) {
        ...
        if (r_ob->notify_with == k_NOTIFY_WITH_REDO_TRANSACTION || r_ob->notify_with == k_NOTIFY_WITH_UNDO_TRANSACTION) {
            // simply updating the paths
        ...
        } else if (r_ob->notify_with == k_NOTIFY_WITH_REDO_TICKS || r_ob->notify_with == k_NOTIFY_WITH_UNDO_TICKS) {
            // In this part we infer the actual sequence of operations that has brought the order of chords to their actual position.
            // We should do the same with markers
            long N = r_ob->undo_information_whose_path_need_to_be_checked->l_size;
            if (N > 0) {
                t_reordering_helper *helper = (t_reordering_helper *)sysmem_newptr(2*N*sizeof(t_reordering_helper));

                // TRICK: we keep the iteration v == r_ob->num_voices for MARKERS!
                for (long v = r_ob->obj_type == k_NOTATION_OBJECT_ROLL ? 0 : r_ob->num_voices; v <= r_ob->num_voices; v++) {
                    ...
                    for (t_llllelem *el = r_ob->undo_information_whose_path_need_to_be_checked->l_head; el; el = el->l_next) {
                        ...
                        if (info->n_it_type == (v == r_ob->num_voices ? k_MARKER : k_CHORD) &&
                            info->modification_type == k_UNDO_MODIFICATION_TYPE_CHANGE_CHECK_ORDER &&
                            info->n_it_path_after.voice_num == (v == r_ob->num_voices ? info->n_it_path_after.voice_num : v)) {
                            ...
                            helper[2*i].position = (v < r_ob->num_voices ? info->n_it_path_after.chord_num : info->n_it_path_after.item_num);
                            helper[2*i].type = 1;
                            ...
                            helper[2*i+1].position = (v < r_ob->num_voices ? info->n_it_path_before.chord_num : info->n_it_path_before.item_num);
                            helper[2*i+1].type = -1;
                    ...
                    for (long i = 0; i < M; i++) {
                        if (v < r_ob->num_voices) {
                            ...
                            if (helper[i].type == 1)
                                helper[i].info->n_it_path_after.item_num = helper[i].position;
                            else if (helper[i].type == -1)
                                helper[i].info->n_it_path_before.item_num = helper[i].position;
                        }
                    }
                }
            }
...
void undo_redo_tick_post(t_notation_obj *r_ob, long what, t_undo_redo_information *info)
{
    ...
    object_post((t_object *) r_ob, " > MODIFICATION = %s", info->modification_type == k_UNDO_MODIFICATION_TYPE_CHANGE ? "change" : (info->modification_type == k_UNDO_MODIFICATION_TYPE_INSERT ? "add" : (info->modification_type == k_UNDO_MODIFICATION_TYPE_REMOVE ? "delete" : (info->modification_type == k_UNDO_MODIFICATION_TYPE_NONE ? "none" : "unknown"))));
...
void undo_op_to_string(long undo_op, char *buf)
{
    switch (undo_op) {
        ...
        case k_UNDO_OP_GENERIC_CHANGE:
            sprintf(buf, "Change");
            break;
        case k_UNDO_OP_GENERIC_TRANSACTION:
            sprintf(buf, "Transaction");
            break;
        ...
        case k_UNDO_OP_CHANGE_MARKER_NAME:
            ...
            break;
        case k_UNDO_OP_CHANGE_REGION_DURATION:
            sprintf(buf, "Change Region Duration");
            break;
...
e_element_types element_type_from_symbol(t_symbol *sym)
{
    ...
    if (sym == _llllobj_sym_durationline)
    ...
    if (sym == _llllobj_sym_breakpointortail)
    ...
    if (sym == _llllobj_sym_mixed)
        return k_MIXED;

    if (sym == _llllobj_sym_timesig)
        return k_TIME_SIGNATURE;

    if (sym == _llllobj_sym_whole)
...
{
    switch (modif_type) {
        case k_UNDO_MODIFICATION_TYPE_NONE: return _llllobj_sym_none; break;
        case k_UNDO_MODIFICATION_TYPE_INSERT: return _llllobj_sym_insert; break;
        case k_UNDO_MODIFICATION_TYPE_REMOVE: return _llllobj_sym_remove; break;
        case k_UNDO_MODIFICATION_TYPE_CHANGE: return _llllobj_sym_change; break;
        case k_UNDO_MODIFICATION_TYPE_CHANGE_CHECK_ORDER: return _llllobj_sym_change; break;
...
{
    if (sym == _llllobj_sym_insert)
        return k_UNDO_MODIFICATION_TYPE_INSERT;
    if (sym == _llllobj_sym_remove)
        return k_UNDO_MODIFICATION_TYPE_REMOVE;
    if (sym == _llllobj_sym_change) {
        ...
        else if (r_ob->obj_type == k_NOTATION_OBJECT_SCORE && (el_type == k_MARKER))
            return k_UNDO_MODIFICATION_TYPE_CHANGE_CHECK_ORDER;
        else
...
t_llll *undo_redo_information_to_llll(t_notation_obj *r_ob, t_undo_redo_information *info)
{
    ...
    if (info->n_it_content) {
        ...
        llll_funall(cloned, (fun_fn) undo_redo_information_to_llll_remove_ids, NULL, 1, cloned->l_depth < 3 ? 1 : -3, FUNALL_PROCESS_WHOLE_SUBLISTS);
...
t_undo_redo_information *undo_redo_information_from_llll(t_notation_obj *r_ob, t_llll *ll)
{
    ...
    if (ll->l_size >= 4) {
        ...
        if (!item && modif_type != k_UNDO_MODIFICATION_TYPE_INSERT && el_type != k_WHOLE_NOTATION_OBJECT && el_type != k_HEADER_DATA) {
            bach_freeptr(info);
            return NULL;
        }
...
t_undo_redo_information *undo_redo_information_reverse(t_notation_obj *r_ob, t_undo_redo_information *this_information)
{
    ...
    if ((!item || item->type != type) && modif_type != k_UNDO_MODIFICATION_TYPE_INSERT && type != k_WHOLE_NOTATION_OBJECT && type != k_HEADER_DATA) {
    ...
    if (param == _llllobj_sym_flag) {
    ...
    } else if (param == _llllobj_sym_slurs) {
        ...
        } else if (modif_type == k_UNDO_MODIFICATION_TYPE_REMOVE) { // measure need to be erased
            newcontent = measure_get_values_as_llll(r_ob, (t_measure *) item, k_CONSIDER_FOR_UNDO, true, true);
//            newcontent = llll_clone(this_information->n_it_content);
            reverse_information = undo_redo_information_create(ID, k_MEASURE, k_UNDO_MODIFICATION_TYPE_INSERT, param, path_after, path_before, k_HEADER_NONE, newcontent);
        } else if (modif_type == k_UNDO_MODIFICATION_TYPE_INSERT) { // measure need to be added
            newcontent = llll_get();
//            newcontent = llll_clone(this_information->n_it_content);
            reverse_information = undo_redo_information_create(ID, k_MEASURE, k_UNDO_MODIFICATION_TYPE_REMOVE, param, path_after, path_before, k_HEADER_NONE, newcontent);
        }
...
//            newcontent = llll_clone(this_information->n_it_content);
            ...
            newcontent = llll_get();
//            newcontent = llll_clone(this_information->n_it_content);
            reverse_information = undo_redo_information_create(ID, k_CHORD, k_UNDO_MODIFICATION_TYPE_REMOVE, param, path_after, path_before, k_HEADER_NONE, newcontent);
        }
    } else if (type == k_MARKER) {
        if (modif_type == k_UNDO_MODIFICATION_TYPE_CHANGE || modif_type == k_UNDO_MODIFICATION_TYPE_CHANGE_CHECK_ORDER) {
            newcontent = marker_get_as_llll(r_ob, (t_marker *)item, false, false, k_CONSIDER_FOR_UNDO);
            ...
            newcontent = marker_get_as_llll(r_ob, (t_marker *)item, false, false, k_CONSIDER_FOR_UNDO);
            reverse_information = undo_redo_information_create(ID, k_MARKER, k_UNDO_MODIFICATION_TYPE_INSERT, param, path_after, path_before, k_HEADER_NONE, newcontent);
        } else if (modif_type == k_UNDO_MODIFICATION_TYPE_INSERT) { // surely roll
            newcontent = llll_get();
            reverse_information = undo_redo_information_create(ID, k_MARKER, k_UNDO_MODIFICATION_TYPE_REMOVE, param, path_after, path_before, k_HEADER_NONE, newcontent);
        }
...
long undo_redo_information_apply(t_notation_obj *r_ob, t_undo_redo_information *this_information, t_llll *measure_whose_flag_needs_to_be_cleared, long *flags)
{
    ...
    if (modif_type == k_UNDO_MODIFICATION_TYPE_CHANGE && param == _llllobj_sym_flag) {
        ...
        if (type == k_WHOLE_NOTATION_OBJECT){
            ...
            } else if (modif_type == k_UNDO_MODIFICATION_TYPE_REMOVE) {
                if (!obj_is_score)
                    (*flags) |= k_UNDO_PERFORM_FLAG_RECOMPUTE_TOTAL_LENGTH;
                delete_marker(r_ob, (t_marker *)item);
                check_correct_scheduling(r_ob, false);
                
            } else if (modif_type == k_UNDO_MODIFICATION_TYPE_INSERT) {
...
bool check_slurs(t_notation_obj *r_ob)
{
    long tot_slurs_from = 0;
    long tot_slurs_to = 0;
    ...
    for (t_voice *voice = r_ob->firstvoice; voice && voice->number < r_ob->num_voices; voice = voice_get_next(r_ob, voice)) {
        for (t_chord *ch = voice_get_first_chord(r_ob, voice); ch; ch = chord_get_next(ch)) {
            tot_slurs_from += ch->num_slurs_from;
            tot_slurs_to += ch->num_slurs_to;
        }
    }
    
    if (tot_slurs_from == tot_slurs_to && tot_slurs_from == r_ob->slurs->l_size) {
        return true;
    } else {
        return false;
    }
...
t_llll *notationobj_last_transaction_to_llll(t_notation_obj *r_ob, char reverse_information, char as_individual_ticks)
{
    t_llll *out = llll_get();
    t_llll *llll = r_ob->undo_llll;

    if (!as_individual_ticks)
        llll_appendsym(out, _llllobj_sym_transaction);
...
long notation_item_get_undo_op(t_notation_obj *r_ob, t_notation_item *item, char modif_type){
    if (item->type == k_MEASURE) {
        switch (modif_type) {
            case k_UNDO_MODIFICATION_TYPE_INSERT:
                return k_UNDO_OP_DELETE_MEASURE;
            case k_UNDO_MODIFICATION_TYPE_REMOVE:
...
t_llll *notation_item_get_values_as_llll_for_undo(t_notation_obj *r_ob, t_notation_item *item){
    if (item->type == k_MEASURE)
        ...
        return marker_get_as_llll(r_ob, (t_marker *)item, false, false, k_CONSIDER_FOR_UNDO);
    }
    return NULL;
}
...
void possibly_add_undo_ticks_for_slurs_from(t_notation_obj *r_ob, t_notation_item *item)
{
    // ADDING INFORMATION FOR SLURS
    if (r_ob->obj_type == k_NOTATION_OBJECT_SCORE && item->type == k_MEASURE) {
        // special case: if a chord inside a measure ends a slur, we need to create an undo tick for the chord that STARTED the slur
        for (t_chord *ch = ((t_measure *)item)->firstchord; ch; ch = ch->next) {
            for (long i = 0; i < ch->num_slurs_from; i++) {
                if (!notation_item_is_under_undo_tick(r_ob, (t_notation_item *)ch->slur_from[i]->start_chord) &&
                    !notation_item_is_under_undo_tick(r_ob, (t_notation_item *)ch->slur_from[i]->start_chord->parent))
                    undo_tick_create_for_notation_item(r_ob, (t_notation_item *)ch->slur_from[i]->start_chord, k_UNDO_MODIFICATION_TYPE_CHANGE, _llllobj_sym_state, false);
            }
        }
    } else if (r_ob->obj_type == k_NOTATION_OBJECT_ROLL && item->type == k_CHORD) {
        t_chord *ch = ((t_chord *)item);
        for (long i = 0; i < ch->num_slurs_from; i++) {
            if (!notation_item_is_under_undo_tick(r_ob, (t_notation_item *)ch->slur_from[i]->start_chord) &&
                !notation_item_is_under_undo_tick(r_ob, (t_notation_item *)ch->slur_from[i]->start_chord->parent))
...
t_undo_redo_information *undo_tick_create_for_notation_item(t_notation_obj *r_ob, t_notation_item *item, e_undo_modification_types modif_type, t_symbol *param, bool automatically_add_undo_ticks_for_slurs_from)
{
    ...
    if (automatically_add_undo_ticks_for_slurs_from)
        possibly_add_undo_ticks_for_slurs_from(r_ob, item);
    
    notation_item_get_path(r_ob, item, &path);
    
    path_before = (modif_type == k_UNDO_MODIFICATION_TYPE_INSERT) ? NULL : &path;
    path_after = (modif_type == k_UNDO_MODIFICATION_TYPE_REMOVE) ? NULL : &path;
    ...
    if (r_ob->notify_with > 1) {
        ...
        for (t_llllelem *el = r_ob->undo_llll->l_head; el && hatom_gettype(&el->l_hatom) != H_LONG; ) {
            ...
            if (existinginfo->n_it_ID == item->ID) {
                if (existinginfo->modification_type == k_UNDO_MODIFICATION_TYPE_REMOVE) {
...
void undo_tick_create_for_selected_notation_item(t_notation_obj *r_ob, t_notation_item *item, e_element_types smallest_undoable_element, e_undo_modification_types modif_type, t_symbol *param, bool automatically_add_undo_ticks_for_slurs_from){
    ...
    if (modif_type == k_UNDO_MODIFICATION_TYPE_CHANGE && param == _llllobj_sym_name && !(undo_item->flags & k_FLAG_MODIF_NAME_UNDO)){
        undo_tick_create_for_notation_item(r_ob, undo_item, k_UNDO_MODIFICATION_TYPE_CHANGE, param, automatically_add_undo_ticks_for_slurs_from);
        ...
        undo_tick_create_for_notation_item(r_ob, undo_item, modif_type, param, automatically_add_undo_ticks_for_slurs_from);
        undo_item->flags = (e_bach_internal_notation_flags) (undo_item->flags | k_FLAG_MODIF_CHECK_ORDER_UNDO);
               ...
               (modif_type == k_UNDO_MODIFICATION_TYPE_INSERT || modif_type == k_UNDO_MODIFICATION_TYPE_REMOVE || !(undo_item->flags & k_FLAG_MODIF_UNDO_WITH_OR_WO_CHECK_ORDER))){
        undo_tick_create_for_notation_item(r_ob, undo_item, modif_type, param, automatically_add_undo_ticks_for_slurs_from);
        undo_item->flags = (e_bach_internal_notation_flags) (undo_item->flags | k_FLAG_MODIF_UNDO);
    }
}

void undo_ticks_create_for_multiple_notation_items(t_notation_obj *r_ob, long num_items, t_notation_item **item, e_undo_modification_types modif_type, t_symbol *param, long *undo_op, bool automatically_add_undo_ticks_for_slurs_from){
    long i;
    ...
    for (i = 0; i < num_items; i++){
        ...
        else if (modif_type == k_UNDO_MODIFICATION_TYPE_CHANGE || modif_type == k_UNDO_MODIFICATION_TYPE_INSERT)
            content = notation_item_get_values_as_llll_for_undo(r_ob, item[i]);
        else
            content = llll_get();
        
        if (automatically_add_undo_ticks_for_slurs_from)
            possibly_add_undo_ticks_for_slurs_from(r_ob, item[i]);
...
void send_changed_bang_and_automessage_if_needed(t_notation_obj *r_ob)
{
    if (r_ob->need_send_changed_bang) {
        ...
        switch (r_ob->notify_with) {
            ...
            case 3:
            {
                t_llll *notifllll = notationobj_last_transaction_to_llll(r_ob, r_ob->notify_with == 2, false);
                llllobj_outlet_llll((t_object *) r_ob, LLLL_OBJ_UI, outlet_number, notifllll);
                llll_free(notifllll);
            }
                break;
            ...
            case 5:
            {
                t_llll *notifllll = notationobj_last_transaction_to_llll(r_ob, r_ob->notify_with == 4, true);
                llllobj_outlet_llll((t_object *) r_ob, LLLL_OBJ_UI, outlet_number, notifllll);
                llll_free(notifllll);
            }
                break;
...
long notationobj_generic_transaction(t_notation_obj *r_ob, t_symbol *msg, long ac, t_atom *av)
{
    ...
    for (t_llllelem *el = ll->l_head; el; el = el->l_next) {
        ...
        t_undo_redo_information *info = undo_redo_information_from_llll(r_ob, hatom_getllll(&el->l_hatom));
        
        if (!info) {
            object_error((t_object *)r_ob, "Can't find notation items.");
        } else {
            t_undo_redo_information *reverse_information = NULL;
            if (add_undo_step)   // reversing information for redo/undo
                reverse_information = undo_redo_information_reverse(r_ob, info);
            
            // applying undo/redo information
            undo_redo_information_apply(r_ob, info, measure_whose_flag_needs_to_be_cleared, &flags);
            
            if (reverse_information)
                undo_redo_tick_create(r_ob, k_UNDO, 0, reverse_information, true);
            
            undo_redo_information_free(info);
        }
    }
    
    if (add_undo_step && undo_ticks_are_dangling(r_ob, false))
        undo_redo_step_marker_create(r_ob, k_UNDO, 0, k_UNDO_OP_GENERIC_TRANSACTION, true);
