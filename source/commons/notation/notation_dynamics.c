...
void dynamics_get_rect(t_notation_obj *r_ob, t_dynamics *dyn, t_rect *enclosure)
{
    ...
    // top and bottom
    double bottom_staff_y = voice_get_staff_bottom_y(r_ob, (r_ob->obj_type == k_NOTATION_OBJECT_ROLL ? (t_voice *) ch->voiceparent : (t_voice *) ch->parent->voiceparent), k_NONSTANDARD_STAFFLINES_TOPBOTTOM_EXTENDONLY);
