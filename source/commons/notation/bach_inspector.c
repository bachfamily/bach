...
void bach_default_postprocess(t_notation_obj *r_ob, void *obj, t_bach_attribute *attr){
	if (attr->owner_type == k_SLOTINFO) {
		...
		if (r_ob->obj_type == k_NOTATION_OBJECT_SCORE) {
            ...
            chord_set_recompute_parameters_flag(((t_chord *)obj));
		...
		if (r_ob->obj_type == k_NOTATION_OBJECT_SCORE)
            ...
            chord_set_recompute_parameters_flag(((t_note *)obj)->parent);
            recompute_total_length(r_ob);
            update_hscrollbar(r_ob, 1);
        }
	} else if (attr->owner_type == k_MEASURE) {
...
{
    t_class *c;
    CLASS_NEW_CHECK_SIZE(c, "bach_inspector_ui", (method)bach_inspector_ui_new, (method)bach_inspector_ui_free, sizeof(t_bach_inspector_ui), 0L, A_GIMME, 0L);
