            ...
            if (view == gensym("line")) {
                ...
                if (r_ob->obj_type == k_NOTATION_OBJECT_ROLL) {
                    if (mspersystem != r_ob->domain && mspersystem > 0) {
                        ...
                        if (adaptwidth) {
                            r_ob->inner_width = r_ob->postdomain_width + mspersystem * (CONST_X_SCALING * r_ob->zoom_x * r_ob->zoom_y) - (r_ob->j_inset_x - get_max_vscrollbar_width_or_inset_x(r_ob) - (get_ux_left_start(r_ob) + r_ob->key_signature_uwidth + r_ob->voice_names_uwidth + notationobj_get_starting_pads_ux(r_ob)) * r_ob->zoom_y);
                        } else {
                            r_ob->zoom_x = (r_ob->inner_width + (r_ob->j_inset_x - get_max_vscrollbar_width_or_inset_x(r_ob) - (get_ux_left_start(r_ob) + r_ob->key_signature_uwidth + r_ob->voice_names_uwidth + notationobj_get_starting_pads_ux(r_ob)) * r_ob->zoom_y) - r_ob->postdomain_width) / (mspersystem * CONST_X_SCALING * r_ob->zoom_y);
                        }
                    }
                    r_ob->inner_height = notationobj_get_supposed_standard_uheight(r_ob) * r_ob->zoom_y;
