...
void score_sel_change_symduration(t_score *x, t_symbol *s, long argc, t_atom *argv)
{
    ...
    set_need_perform_analysis_and_change_flag((t_notation_obj *)x);
    perform_analysis_and_change(x, NULL, NULL, NULL, flags);
...
void score_sel_append_slot_item(t_score *x, t_symbol *s, long argc, t_atom *argv)
{
    t_llll *args = llllobj_parse_llll((t_object *) x, LLLL_OBJ_UI, NULL, argc, argv, LLLL_PARSE_RETAIN);
    if (args->l_head)
...
void score_sel_prepend_slot_item(t_score *x, t_symbol *s, long argc, t_atom *argv)
{
    t_llll *args = llllobj_parse_llll((t_object *) x, LLLL_OBJ_UI, NULL, argc, argv, LLLL_PARSE_RETAIN);
    if (args->l_head)
        llll_insertlong_after(1, args->l_head); // inserting dummy position
...
void score_sel_delete_slot_item(t_score *x, t_symbol *s, long argc, t_atom *argv)
{
    t_llll *args = llllobj_parse_llll((t_object *) x, LLLL_OBJ_UI, NULL, argc, argv, LLLL_PARSE_RETAIN);
	...
	if (what_to_dump_is_empty || is_symbol_in_llll_first_level(what_to_dump, _llllobj_sym_body)) {
		...
		for (voice = x->firstvoice;	voice && voice->v_ob.number < x->r_ob.num_voices; voice = voice->next) {
			...
			while (cur_ch[voice_num] && cur_ch[voice_num]->r_sym_duration.r_num < 0) {
				cur_ch[voice_num] = chord_get_next(cur_ch[voice_num]);
			}
		}
			...
			for (chord = meas->firstchord; chord; chord = chord->next){
				if (chord->r_sym_duration.r_num < 0) { // rest
					if (notation_item_is_globally_selected((t_notation_obj *) x, (t_notation_item *)chord) && last_nonrest_chord_llll) {
						...
						prevchord = chord_get_prev(chord);
			...
			if (scheduled_item_type == k_CHORD || scheduled_item_type == k_MARKER || scheduled_item_type == k_MEASURE || scheduled_item_type == k_TEMPO) {
                ...
                if (x->r_ob.play_measures) {
                    for (voice = x->firstvoice; voice && voice->v_ob.number < x->r_ob.num_voices && count < x->r_ob.max_num_chord_per_scheduler_event; voice = voice->next){
                        for (measure = x->r_ob.measure_play_cursor[voice->v_ob.number] ? measure_get_next(x->r_ob.measure_play_cursor[voice->v_ob.number]) : voice->firstmeasure;
                             measure && count < x->r_ob.max_num_chord_per_scheduler_event; measure = measure_get_next(measure)){
                ...
                if (x->r_ob.play_tempi) {
                    for (voice = x->firstvoice; voice && voice->v_ob.number < x->r_ob.num_voices && count < x->r_ob.max_num_chord_per_scheduler_event; voice = voice->next){
                        for (tempo = x->r_ob.tempo_play_cursor[voice->v_ob.number] ? tempo_get_next(x->r_ob.tempo_play_cursor[voice->v_ob.number]) : (voice->firstmeasure ? voice->firstmeasure->firsttempo : NULL);
                             tempo && count < x->r_ob.max_num_chord_per_scheduler_event; tempo = tempo_get_next(tempo)){
				...
				for (voice = x->firstvoice; voice && voice->v_ob.number < x->r_ob.num_voices && count < x->r_ob.max_num_chord_per_scheduler_event; voice = voice->next){
					for (temp_ch = x->r_ob.chord_play_cursor[voice->v_ob.number] ? chord_get_next(x->r_ob.chord_play_cursor[voice->v_ob.number]) : (voice->firstmeasure ? voice->firstmeasure->firstchord : NULL);
                         temp_ch && count < x->r_ob.max_num_chord_per_scheduler_event; temp_ch = chord_get_next(temp_ch)){
...
//	jbox_initclass(c, JBOX_COLOR | JBOX_FIXWIDTH | JBOX_FONTATTR);
	...
	// @method changeslotitem @digest Change a specific slot element inside a given slot of selected items
...
void score_lambda(t_score *x, t_symbol *s, long argc, t_atom *argv){ 
	if (argc && atom_gettype(argv) == A_SYM){
        ...
        if (router == _llllobj_sym_cents){
			...
			score_sel_change_slot_item(x, _llllobj_sym_lambda, argc - 1, argv + 1);
        } else if (router == _llllobj_sym_changeslotitem){
            score_sel_change_slot_item(x, _llllobj_sym_lambda, argc - 1, argv + 1);
        } else if (router == _llllobj_sym_appendslotitem){
            score_sel_append_slot_item(x, _llllobj_sym_lambda, argc - 1, argv + 1);
        } else if (router == _llllobj_sym_prependslotitem){
            score_sel_prepend_slot_item(x, _llllobj_sym_lambda, argc - 1, argv + 1);
        ...
        if (inputlist && (inputlist->l_size > 0)) {
            ...
            if (!x->must_append_measures || (x->must_append_measures && !router)) { // should be always
                if (hatom_gettype(&firstelem->l_hatom) == H_LLLL || !router || (router == _llllobj_sym_score)) {
                    ...
                    } else if (s == _llllobj_sym_bach_llll){ // native llll: we might try to deparse it and send it as textual
...
void overtype_voice(t_score *x, t_scorevoice *voice, t_timepoint *from_here, t_timepoint *to_here, t_llll *new_content)
{
    if (voice && voice->v_ob.number < x->r_ob.num_voices && from_here && new_content && new_content->l_size > 0) {
        ...
        t_llll *measureinfo = llll_from_text_buf("(4 4)");
...
//			perform_analysis_and_change(x, NULL, NULL, k_BEAMING_CALCULATION_DONT_CHANGE_CHORDS);
	...
	if (changed){
		set_need_perform_analysis_and_change_flag((t_notation_obj *)x);
		perform_analysis_and_change(x, NULL, NULL, NULL, k_BEAMING_CALCULATION_DONT_CHANGE_CHORDS + k_BEAMING_CALCULATION_DONT_AUTOCOMPLETE);
	}
...
char rebeam_levels_of_selected_tree_nodes(t_score *x, char also_destroy_tuplets, char force_autoparse, long flags)
{
	...
	if (changed) {
        ...
        perform_analysis_and_change(x, NULL, NULL, NULL, flags); //k_BEAMING_CALCULATION_DONT_AUTOCOMPLETE);
        if (force_autoparse)
						...
						if (is_in_durationline_shape((t_notation_obj *)x,curr_nt,this_x,this_y)) {
							if ((modifiers == eAltKey) && (x->r_ob.allow_glissandi) && (!notation_item_is_globally_locked((t_notation_obj *) x, (t_notation_item *)curr_nt))) { 
								...
								if (is_editable((t_notation_obj *)x, k_PITCH_BREAKPOINT, k_CREATION)) {
                                    ...
                                    t_note *last_tied;
                                    double end_x;
                                    if (x->r_ob.dl_spans_ties && (last_tied = note_get_last_in_tieseq(curr_nt)))
...
long score_keyfilter(t_score *x, t_object *patcherview, long *keycode, long *modifiers, long *textcharacter)
{
	...
	if (k == JKEY_TAB) {
		if (x->r_ob.is_editing_type == k_LYRICS && x->r_ob.is_editing_chord && x->r_ob.is_editing_chord->firstnote) {
			char *text	= NULL;
			long size = 0;
			t_chord *prev_chord = chord_get_prev_nonrest(x->r_ob.is_editing_chord);
			t_chord *next_chord = chord_get_next_nonrest(x->r_ob.is_editing_chord);
...
t_chord *chord_get_last_before_notation_cursor(t_score *x){
	t_measure *meas = x->r_ob.notation_cursor.measure;
				...
				case JKEY_LEFTARROW: 
				{
					...
					if (modifiers & eCommandKey) {
						...
						if (!x->r_ob.notation_cursor.chord) {
							...
							t_chord *prev_chord = chord_get_prev(x->r_ob.notation_cursor.chord);
					...
					if (modifiers & eCommandKey) {
						...
						if (!x->r_ob.notation_cursor.chord) {	// gotta jump to next measure
							...
							if (!x->r_ob.notation_cursor.chord->next && x->r_ob.notation_cursor.measure->measure_filling < 0){
								...
								t_chord *next_chord = chord_get_next(x->r_ob.notation_cursor.chord);
	...
	while (curr_it) { // cycle on the selected items
		if ((curr_it->type == k_PITCH_BREAKPOINT) && ((t_bpt *)curr_it)->next && !tail_only) { // it is a breakpoint : let's move it
			...
			if (!notation_item_is_globally_locked((t_notation_obj *) x, (t_notation_item *)note)) {
                double note_length;
                t_note *last_tied;
                if (x->r_ob.dl_spans_ties && (last_tied = note_get_last_in_tieseq(note)))
	...
	while (temp) {
		...
		if (temp->type == k_MEASURE) {
			if (!notation_item_is_globally_locked((t_notation_obj *) x, temp)) {
                char need_update_solos = false;
                if (voiceensemble_delete_measure(x, (t_measure *)temp, ((t_measure *)temp)->firstchord ? chord_get_prev(((t_measure *)temp)->firstchord) : NULL, &need_update_solos, true))
...
*/			
    ...
    recompute_all(x);
    perform_analysis_and_change(x, NULL, NULL, NULL, 0);
	unlock_general_mutex((t_notation_obj *)x);
