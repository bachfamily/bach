...
#define CONST_COLLAPSE_SLOT (CONST_MAX_SLOTS - 1)
//#define CONST_COLLAPSE_SLOT 5 // (CONST_MAX_SLOTS + 1) // "fake" slot where we store the voice information of collapsed notes
...
void merge_small_notes_inplace(t_quantize *x, t_llll **rhythm, t_llll **infos, t_llll **ties, 
								...
								t_llll *minimal_units);
void destroy_rhythm_element(t_quantize *x, t_llllelem *rhythm_elem_to_delete, t_llllelem *infos_elem_to_delete, t_llllelem *ties_elem_to_delete, 
							t_llllelem *prev_elem_rhythm, t_llllelem *prev_elem_infos, t_llllelem *prev_elem_ties, 
							t_llllelem *next_elem_rhythm, t_llllelem *next_elem_infos, t_llllelem *next_elem_ties, 
...
void quantize_collapse(t_llll **measureinfo, t_llll **cents, t_llll **durations, t_llll **velocities, t_llll **ties, t_llll **graphic, t_llll **breakpoints, 
						t_llll **slots, t_llll **regularityboxes, long collapse_voice_slot_num, long num_voices, 
...
t_max_err quantize_setattr_minimal_units(t_quantize *x, t_object *attr, long ac, t_atom *av){
	...
	if (ac && av) {
		...
		if (!x->minimal_units || x->minimal_units->l_size == 0) {
			...
			llll_appendrat(x->minimal_units, RAT_1OVER16, 0, WHITENULL_llll);
...
int T_EXPORT main()
{
	...
	c = class_new("bach.quantize", (method)quantize_new, (method)quantize_free, (short)sizeof(t_quantize), 0L, A_GIMME, 0);
	
	// @method llll @digest Function depends on inlet
	// @description 
	// In first inlet: Sets the measureinfo and performs the quantization (outputting the result). <br />
	// In second inlet: Receives the result of bach.roll's "quantize" message (from bach.roll's or bach.score's first outlet). In "separate" mode: Sets the Pitches or cents to be quantized. <br />
		...
		if (role == k_MARKER_ROLE_TEMPO) {
			...
			if (content) {
				...
				if (onset >= from_ms - CONST_THRESHOLD_MS_FOR_TEMPO_MARKERS_BEING_ON_TS_MARKERS && onset <= from_ms + CONST_THRESHOLD_MS_FOR_TEMPO_MARKERS_BEING_ON_TS_MARKERS) {
					...
					llll_appendrat(thistempo, tempo_figure, 0, WHITENULL_llll);
					llll_appendlong(thistempo, round(tempo), 0, WHITENULL_llll);
					llll_appendllll(out, thistempo, 0, WHITENULL_llll);
					...
					llll_appendrat(thistempo, tempo_figure, 0, WHITENULL_llll);
					llll_appendlong(thistempo, round(tempo), 0, WHITENULL_llll);
					llll_appendrat(thistempo, changepoint, 0, WHITENULL_llll);
					llll_appendllll(out, thistempo, 0, WHITENULL_llll);
...
t_llll *get_nilinfo(){
	...
	llll_appendllll(out, llll_get(), 0, WHITENULL_llll);
	llll_appendllll(extras, llll_get(), 0, WHITENULL_llll);
	llll_appendllll(extras, llll_get(), 0, WHITENULL_llll);
	llll_appendllll(extras, llll_get(), 0, WHITENULL_llll);
	llll_appendllll(out, extras, 0, WHITENULL_llll);
	return out;
}
				...
				if (role == k_MARKER_ROLE_MEASURE_BARLINE || role == k_MARKER_ROLE_TIME_SIGNATURE) {
					...
					for (temp = elem->l_next; temp && num_divs < MAX_NUM_DIVISIONS; temp = temp->l_next) {
                        ...
                        if (this_role == k_MARKER_ROLE_MEASURE_SUBDIVISION) {
							...
							num_divs++;
                            tot_num_subdivs++;
							llll_appenddouble(onsets_divs, get_marker_onset_from_llllelem(temp), 0, WHITENULL_llll);
					...
					for (i = 1, divel = onsets_divs->l_head; divel && i < num_divs; divel = divel->l_next, i++) {
						...
						llll_appenddouble(this_ll, cur, 0, WHITENULL_llll);
						llll_appenddouble(this_ll, this_onset - cur, 0, WHITENULL_llll);
                        llll_appendrat(this_ll, this_div_sym_dur, 0, WHITENULL_llll);
                        llll_appendllll(turnpike, this_ll, 0, WHITENULL_llll);
                        
                        llll_appenddouble(divisions_absolute_position, this_onset);
						
						llll_appenddouble(quant_boxes_ms, this_onset - cur, 0, WHITENULL_llll);
						llll_appendrat(quant_boxes, this_div_sym_dur, 0, WHITENULL_llll);
                        llll_appenddouble(quant_boxes_quartertempo, 240000. * rat2double(this_div_sym_dur)/(this_onset - cur), 0, WHITENULL_llll);
					...
					if (num_barlines_in_range > 1) { // more than one barline inside!
						...
						for (i = 1; i < num_barlines_in_range; i++) {
							...
							for (meas_boxes_elem = meas_boxes->l_tail; meas_boxes_elem; meas_boxes_elem = meas_boxes_elem->l_prev){
								...
								llll_appenddouble(this_ll, box_cur, 0, WHITENULL_llll);
								llll_appenddouble(this_ll, box_cur + this_box_dur, 0, WHITENULL_llll);
								llll_appendrat(this_ll, this_box_sym_dur, 0, WHITENULL_llll);
								llll_appendllll(turnpike, this_ll, 0, WHITENULL_llll);
								
                                llll_appenddouble(divisions_absolute_position, box_cur);

                                llll_appenddouble(quant_boxes_ms, this_box_dur, 0, WHITENULL_llll);
								llll_appendrat(quant_boxes, this_box_sym_dur, 0, WHITENULL_llll);
                                llll_appenddouble(quant_boxes_quartertempo, 240000. * rat2double(this_box_sym_dur)/this_box_dur, 0, WHITENULL_llll);
                                llll_appendrat(quant_boxes_tempofigures, curr_tempo_figure);
							...
							llll_appendrat(measure_boxes, ts_dur, 0, WHITENULL_llll);
							llll_appenddouble(measure_boxes_ms, single_meas_dur, 0, WHITENULL_llll);
                            ...
                            llll_appendllll(voice_ts, this_measureinfo_ll, 0, WHITENULL_llll);
					...
					if (num_divs == 1) {
                        ...
                        final_llll = llll_get();
						llll_appenddouble(final_llll, cur, 0, WHITENULL_llll);
						llll_appenddouble(final_llll, next_onset, 0, WHITENULL_llll);
						llll_appendrat(final_llll, this_div_sym_dur, 0, WHITENULL_llll);
						llll_appendllll(turnpike, final_llll, 0, WHITENULL_llll);

                        llll_appenddouble(divisions_absolute_position, cur);

						llll_appenddouble(quant_boxes_ms, next_onset - cur, 0, WHITENULL_llll); // quant_boxes in ms
						llll_appendrat(quant_boxes, this_div_sym_dur, 0, WHITENULL_llll); // quant_boxes in symbolic
                        llll_appenddouble(quant_boxes_quartertempo, 240000. * rat2double(this_div_sym_dur)/(next_onset - cur), 0, WHITENULL_llll);
                        llll_appendrat(quant_boxes_tempofigures, curr_tempo_figure);
					...
					llll_appendrat(measure_boxes, ts_dur, 0, WHITENULL_llll);
					llll_appenddouble(measure_boxes_ms, next_onset - onset, 0, WHITENULL_llll);
...
//                        curr_figure_tempo_value = rat2double(rat_rat_div(ts_to_sym_duration(&curr_ts), curr_tempo_figure)) * (0.25/rat2double(curr_tempo_figure)) * 60 * 1000. / (next_onset - onset);
                    ...
                    tselem = llll_appendllll(voice_ts, this_measureinfo_ll, 0, WHITENULL_llll);
						...
						if (onset >= CONST_THRESHOLD_MS_FOR_PICKUP_MEASURE * dur/quant_boxes->l_size) { // fixed threshold expressed as a ratio of the division
							...
							while (cur > 0) {
								...
								for (meas_boxes_elem = meas_boxes->l_tail; meas_boxes_elem; meas_boxes_elem = meas_boxes_elem->l_prev){
									...
									llll_appenddouble(this_ll, box_cur - this_box_dur, 0, WHITENULL_llll);
									llll_appendrat(this_ll, this_box_sym_dur, 0, WHITENULL_llll);
									llll_prependllll(turnpike, this_ll, 0, WHITENULL_llll);
									
                                    llll_prependdouble(divisions_absolute_position, box_cur);

                                    llll_prependdouble(quant_boxes_ms, this_box_dur, 0, WHITENULL_llll);  // quant_boxes is in ms
									llll_prependrat(quant_boxes, this_box_sym_dur, 0, WHITENULL_llll);
                                    llll_prependdouble(quant_boxes_quartertempo, 240000. * rat2double(this_box_sym_dur)/this_box_dur, 0, WHITENULL_llll);
                                    llll_prependrat(quant_boxes_tempofigures, curr_tempo_figure);
									box_cur -= this_box_dur;
								}
								
								// measure-wise information
								llll_prependrat(measure_boxes, ts_dur, 0, WHITENULL_llll);
								llll_prependdouble(measure_boxes_ms, dur, 0, WHITENULL_llll);
                                ...
                                llll_appendllll(this_measureinfo_ll, this_ts_ll);
                                llll_appendllll(this_measureinfo_ll, this_tempi_ll);
								tselem = llll_prependllll(voice_ts, this_measureinfo_ll, 0, WHITENULL_llll);
							...
							*global_delta_onset = -cur;
							...
							llll_prependdouble(voice_durations, cur, 0, WHITENULL_llll);
							llll_prependllll(voice_infos, get_nilinfo(), 0, WHITENULL_llll);
							llll_prependllll(voice_ties, llll_get(), 0, WHITENULL_llll);
							...
							*global_delta_onset = -onset;
							...
							// adding a fake rest at the beginning
							llll_prependdouble(voice_durations, onset, 0, WHITENULL_llll);
							llll_prependllll(voice_infos, get_nilinfo(), 0, WHITENULL_llll);
							llll_prependllll(voice_ties, llll_get(), 0, WHITENULL_llll);
	...
	if (quant_boxes_ms->l_tail && quant_boxes->l_tail && voice_ts->l_tail) {
		...
		while (tot_boxed_duration < sum_all_durations) {
			...
			for (meas_boxes_elem = meas_boxes->l_tail; meas_boxes_elem; meas_boxes_elem = meas_boxes_elem->l_prev){
				...
				llll_appenddouble(quant_boxes_ms, this_box_dur, 0, WHITENULL_llll);  // quant_boxes is in ms
				llll_appendrat(quant_boxes, this_box_sym_dur, 0, WHITENULL_llll); 
                llll_appenddouble(quant_boxes_quartertempo, 240000. * rat2double(this_box_sym_dur)/this_box_dur, 0, WHITENULL_llll);
                llll_appendrat(quant_boxes_tempofigures, curr_tempo_figure);
				box_cur += this_box_dur;
			...
			//measure-wise information
			llll_appendrat(measure_boxes, last_ts_dur, 0, WHITENULL_llll);
			llll_appenddouble(measure_boxes_ms, last_measure_dur, 0, WHITENULL_llll);

			t_llll *new_last_ll = llll_get();
			llll_appendhatom_clone(new_last_ll, &voice_ts->l_tail->l_hatom.h_w.w_llll->l_head->l_hatom, 0, WHITENULL_llll);
			llll_appendllll(voice_ts, new_last_ll, 0, WHITENULL_llll);
            llll_appendllll(voice_ts->l_tail->l_hatom.h_w.w_llll, tempi_handling == k_TEMPI_FROM_TEMPO_MARKERS ? markers_to_tempi(x, markers, tot_boxed_duration, tot_boxed_duration + last_measure_dur, last_ts_dur, curr_tempo_figure) : llll_get(), 0, WHITENULL_llll);
...
//			dev_llll_print(voice_ts, NULL, 0, 2, NULL);
	...
	llll_appendlong(foo, 4, 0, WHITENULL_llll); 
	llll_appendlong(foo, 4, 0, WHITENULL_llll);
	llll_appendllll(foo2, foo, 0, WHITENULL_llll);
	...
	for (durs_box = (*unquantized_boxed_voice_durations)->l_head, widths_box = turnpike->l_head; durs_box && widths_box; durs_box = durs_box->l_next, widths_box = widths_box->l_next) {
		...
		llll_appendobj(foo5, dummytempo, 0, WHITENULL_llll);
...
void quantize_anything(t_quantize *x, t_symbol *msg, long ac, t_atom *av)
{
			...
			while (arguments->l_head && hatom_gettype(&arguments->l_head->l_hatom) == H_LLLL && hatom_getllll(&arguments->l_head->l_hatom)->l_head && hatom_gettype(&hatom_getllll(&arguments->l_head->l_hatom)->l_head->l_hatom) == H_SYM){
				if (hatom_getsym(&hatom_getllll(&arguments->l_head->l_hatom)->l_head->l_hatom) == _llllobj_sym_markers){
					...
					// 1. copying markers to header_info
				  	llll_appendhatom_clone(x->header_info, &arguments->l_head->l_hatom, 0, WHITENULL_llll);
	...
	if (inlet == 0) {
		...
		long i, v;
		t_llll *inlist[7];
		t_llll *measures, *cents, *durations, *velocities, *ties, *extras, *regularityboxes;
		...
		if (measures && measures->l_head) { // measures have to be defined!!!
			...
			t_llllelem *ties_elem, *meas_elem, *durations_elem, *cents_elem, *velocities_elem, *graphic_elem, *breakpoints_elem, *slots_elem, *regularityboxes_elem;
			t_llll *boxed_extras;
			...
			if (regularityboxes && regularityboxes->l_tail)
				for (j = 1; j <= num_repetitions; j++)
					llll_appendhatom_clone(regularityboxes, &regularityboxes->l_tail->l_hatom, 0, WHITENULL_llll);
			
			
			
			// handling the collapse case
			if (collapse){
				quantize_collapse(&measures, &cents, &durations, &velocities, &ties, &graphic, &breakpoints, &slots, &regularityboxes, CONST_COLLAPSE_SLOT, num_voices,
									&need_free_graphic, &need_free_breakpoints, &need_free_slots);
			...
			for (cents_elem = cents->l_head, velocities_elem = velocities->l_head, regularityboxes_elem = regularityboxes->l_head,
				 ...
				 cents_elem || velocities_elem || durations_elem || meas_elem || ties_elem || graphic_elem || breakpoints_elem || slots_elem;
				...
				t_llllelem *voice_cents_elem, *voice_velocities_elem, *voice_graphic_elem, *voice_breakpoints_elem, *voice_slots_elem, *voice_ties_elem;
				...
				t_llll *boxed_voice_cents, *boxed_voice_velocities, *boxed_voice_graphic, *boxed_voice_breakpoints, *boxed_voice_slots;
...
//				char debug1[1000], debug2[1000], debug3[1000];
				...
				for (voice_cents_elem = (voice_cents) ? voice_cents->l_head : NULL, 
					 ...
					 voice_cents_elem || voice_velocities_elem || voice_graphic_elem || voice_breakpoints_elem || voice_slots_elem || voice_ties_elem;
				...
				if (auto_measures && x->marker_info) {
					...
					if (no_barline_markers) {
						...
						llll_appenddouble(fake_barline_marker, first_tempo_marker_onset, 0, WHITENULL_llll);
						llll_appendsym(fake_barline_marker, _llllobj_sym_barline, 0, WHITENULL_llll);
						llll_appendsym(fake_barline_marker, _llllobj_sym_barline, 0, WHITENULL_llll);
						llll_appendllll(fake_barline_marker, llll_get(), 0, WHITENULL_llll);
						if (!firsttempo)
							llll_appendllll(x->marker_info, fake_barline_marker, 0, WHITENULL_llll);
						else
							llll_insertllll_after(fake_barline_marker, firsttempo, 0, WHITENULL_llll);
					}
					...
					llll_appendllll(measures, voice_ts, 0, WHITENULL_llll);
					...
					while (measures_elem) {
													...
													for (thiselem2 = whole_tempi->l_head; thiselem2; thiselem2 = thiselem2->l_next) {
														if (hatom_gettype(&thiselem2->l_hatom) == H_LLLL) {
															...
															if (thistempo) {
																...
																llll_appendobj(voice_tempi, thistempo, 0, WHITENULL_llll);
															}
														}
													}
												}
						...
						for (j = 0; j < loop_for_n_times; j++) {
							...
							llll_appendlong(this_ts_llll, num, 0, WHITENULL_llll);
							llll_appendlong(this_ts_llll, den, 0, WHITENULL_llll);
							llll_appendllll(voice_ts, this_ts_llll, 0, WHITENULL_llll);
							
							llll_appendrat(measure_boxes, this_measure_sym_duration, 0, WHITENULL_llll);
						}
						if (measures_elem) 
					...
					for (j = 0; j < how_many_measures_to_append && measure_boxes->l_tail; j++) {
						llll_appendhatom_clone(measure_boxes, &measure_boxes->l_tail->l_hatom, 0, WHITENULL_llll);
						llll_appendhatom_clone(voice_ts, &voice_ts->l_tail->l_hatom, 0, WHITENULL_llll);
					}
					...
					for (melem = voice_ts->l_head, voice_regularityboxes_elem = voice_regularityboxes ? voice_regularityboxes->l_head : NULL; 
						...
						llll_appendllll(quant_boxes, these_quant_boxes, 0, WHITENULL_llll);
...
*/
				...
				for (boxed_voice_durations_llllelem = unquantized_boxed_voice_durations->l_head, 
					 ...
					 quant_boxes_llllelem = quant_boxes_llllelem->l_next) {
...
//					llll_to_char_array(hatom_getllll(&boxed_voice_ties_llllelem->l_hatom), debug3, 999);
					...
					if (quantized_box_durations && quantized_box_durations->l_size > 0) 
						llll_appendllll(quantized_durations, quantized_box_durations, 0, WHITENULL_llll);
					else 
						llll_appendllll(quantized_durations, get_nilnil(), 0, WHITENULL_llll);
					
					if (quantized_box_infos && quantized_box_infos->l_size > 0) 
						llll_appendllll(quantized_infos, quantized_box_infos, 0, WHITENULL_llll);
					else
						llll_appendllll(quantized_infos, get_nilnil(), 0, WHITENULL_llll);

					if (quantized_box_ties && quantized_box_ties->l_size > 0) 
						llll_appendllll(quantized_ties, quantized_box_ties, 0, WHITENULL_llll);
					else
						llll_appendllll(quantized_ties, get_nilnil(), 0, WHITENULL_llll);
 			...
 			if (x->output_separate) {
				if (collapse) {
					...
					while (measures->l_head && (long)measures->l_size < num_voices)
						llll_appendhatom_clone(measures, &measures->l_head->l_hatom, 0, WHITENULL_llll);
				...
				llll_prependsym(gathered, _llllobj_sym_score, 0, WHITENULL_llll);
				...
				if (x->header_info->l_size > 0) {
					llll_prependllll_clone(gathered, x->header_info, 0, WHITENULL_llll, NULL);
						...
						char *need_free_graphic, char *need_free_breakpoints, char *need_free_slots) 
{
	...
	llll_appendllll_clone(info, *cents, 0, WHITENULL_llll, NULL);
	llll_appendllll_clone(info, *velocities, 0, WHITENULL_llll, NULL);
	llll_appendllll_clone(info, *ties, 0, WHITENULL_llll, NULL);
	llll_appendllll_clone(info, *graphic, 0, WHITENULL_llll, NULL);
	llll_appendllll_clone(info, *breakpoints, 0, WHITENULL_llll, NULL);
	llll_appendllll_clone(info, *slots, 0, WHITENULL_llll, NULL);
	...
	t_llll *tot = llll_get();
	llll_appendllll_clone(tot, *durations, 0, WHITENULL_llll, NULL);
	llll_appendllll(tot, info, 0, WHITENULL_llll);
							...
							for (note_el = chord_ll->l_head; note_el; note_el = note_el->l_next) {
								if (hatom_gettype(&note_el->l_hatom) == H_LLLL) {
									...
									if (slots_el && hatom_gettype(&slots_el->l_hatom) == H_LLLL) {
										...
										llll_appendlong(newlist, collapse_voice_slot_num, 0, WHITENULL_llll);
										llll_appendlong(newlist, i, 0, WHITENULL_llll);
										llll_appendllll(slots_ll, newlist, 0, WHITENULL_llll);
									}
								}
							}
						}
...
//	llll_to_text_buf(tot, &buf5, 0, 2, 0, NULL);
	...
	while (true) {
		...
		llll_appenddouble(this_lapse, (best_onset - cur_onset_general)*(is_rest ? -1 : 1), 0, WHITENULL_llll);
		...
		llll_appendllll(this_lapse, this_lapse_notes, 0, WHITENULL_llll);
		llll_appendllll(collapsed, this_lapse, 0, WHITENULL_llll);
...
//	llll_post(gathered, 0, 1, 2, NULL, NULL);
	...
	if (gathered && gathered->l_head) {
	    ...
	    while ((long)gathered->l_size < num_voices)
			llll_appendhatom_clone(gathered, &gathered->l_head->l_hatom, 0, WHITENULL_llll);
...
/*		t_llll *default_minimal_units_llll = llll_get();
		llll_appendrat(default_minimal_units_llll, RAT_1OVER16, 0, WHITENULL_llll);
		llll_appendrat(default_minimal_units_llll, genrat(1,12), 0, WHITENULL_llll);
...
//		x->quantization_priority = k_NOTES_FIRST_DECREASING;
		...
		if (!x->minimal_units || x->minimal_units->l_size == 0) {
			if (x->minimal_units) llll_free(x->minimal_units);
			x->minimal_units = llll_get();
			llll_appendrat(x->minimal_units, RAT_1OVER16, 0, WHITENULL_llll); // default
			x->num_minimal_units = 1;
			x->use_dynamic_minimal_units = 0;
		} 
...
t_llll *get_approx_for_elem(t_quantize *x, t_llllelem *elem, t_llll *minimal_units)
{
		...
		for (i = -x->approx_width; i <= x->approx_width; i++) {
			if (quoto + i > 0) {
				...
				for (j = 0; j < mu_number; j++)
					llll_appendlong(tree_address, (j == mu_index) ? quoto+i : 0, 0, WHITENULL_llll);
				llll_appendrat(inner_llll, error, 0, WHITENULL_llll);
				llll_appendrat(inner_llll, approx, 0, WHITENULL_llll);
				llll_appendllll(inner_llll, tree_address, 0, WHITENULL_llll);
				
				llll_appendllll(approx_for_this_elem, inner_llll, 0, WHITENULL_llll);
			}
		}
	}
			...
			if (get_num_zeros_in_long_array(multiindex, mu_number) < mu_number-1) {
				...
				if (rat_rat_cmp(error, worst_error) <= 0) {
					...
					for (i = 0; i < mu_number; i++)
						llll_appendlong(tree_address, multiindex[i], 0, WHITENULL_llll);
	...
	inner_llll = llll_get();
	llll_appendrat(inner_llll, elem_rat, 0, WHITENULL_llll);
	llll_appendrat(inner_llll, long2rat(0), 0, WHITENULL_llll);
	llll_appendllll(inner_llll, repeat_long(0, mu_number), 0, WHITENULL_llll);
	llll_appendllll(approx_for_this_elem, inner_llll, 0, WHITENULL_llll);
...
char is_approx_ok_for_allow_and_deny_box_durs(t_quantize *x, t_llllelem **elem_multiindex, long up_to_depth){
	...
	for (i = 0; i <= up_to_depth; i++)
		llll_appendrat(temp, approx_elem_to_approx(elem_multiindex[i]), 0, WHITENULL_llll);
	...
	*quantized_box_durations = llll_get(); 
	...
	for (elem = box_durations->l_head; elem; elem = elem->l_next) {
		if (hatom_getrational(&elem->l_hatom).r_num > 0) {
			llll_appendlong(signs, 1, 0, WHITENULL_llll);
		} else {
			llll_appendlong(signs, -1, 0, WHITENULL_llll);
	...
	for (elem = box_durations->l_head; elem; elem = elem->l_next) {
		...
		llll_appendllll(approx_possib, this_elem_approx, 0, WHITENULL_llll);
	}
...
//		for (elem = box_durations->l_head; is_ok && depth < num_durations && elem && is_ok; depth++, elem = elem->l_next) {
		for (; is_ok && depth < num_durations; depth++) {
			...
			llll_appendrat(curr_durations, elem_approx, 0, WHITENULL_llll);
...
void regroup_delete_fastest_element(t_quantize *x, t_llll *rhythm, t_llll *infos, t_llll *ties, 
	...
	if (rhythm && rhythm->l_size > 1) {
		...
		destroy_rhythm_element(x, smallest_rhythm_elem, smallest_infos_elem, smallest_ties_elem, 
...
void destroy_rhythm_element(t_quantize *x, t_llllelem *rhythm_elem_to_delete, t_llllelem *infos_elem_to_delete, t_llllelem *ties_elem_to_delete, 
	...
	t_llll *these_ties = NULL, *these_mc = NULL, *these_infos = NULL;
	t_llllelem *mcelem;
	...
	t_llll *prev_ties = NULL, *prev_mc = NULL, *prev_infos = NULL;
	t_llll *next_ties = NULL, *next_mc = NULL, *next_infos = NULL;
	...
	if (turn_into_grace_chord) {
		...
		if (mcelem && hatom_gettype(&mcelem->l_hatom) == H_LLLL) {
			...
			llll_appenddouble(these_mc, hatom_getdouble(&mcelem->l_hatom), 0, WHITENULL_llll);
		} else { // rest
			llll_appendllll(these_mc, llll_get(), 0, WHITENULL_llll);
		}
		...
		if (prev_elem_infos && prev_elem_ties && next_elem_infos && next_elem_ties) {
			t_llllelem *mcelem = info_to_midicents(prev_elem_infos);
			if (mcelem && hatom_gettype(&mcelem->l_hatom) == H_LLLL) {
				systhread_mutex_lock(x->n_mutex);
				llll_clone_upon(hatom_getllll(&mcelem->l_hatom), prev_mc);
				systhread_mutex_unlock(x->n_mutex);
			} else if (mcelem) {
				llll_appenddouble(prev_mc, hatom_getdouble(&mcelem->l_hatom), 0, WHITENULL_llll);
			} else { // rest
				llll_appendllll(prev_mc, llll_get(), 0, WHITENULL_llll);
			}
		...
		if (next_elem_infos && next_elem_ties) {
			...
			if (mcelem && hatom_gettype(&mcelem->l_hatom) == H_LLLL) {
				...
				llll_appenddouble(next_mc, hatom_getdouble(&mcelem->l_hatom), 0, WHITENULL_llll);
			} else { // rest
				llll_appendllll(next_mc, llll_get(), 0, WHITENULL_llll);
			}
	...
	if (!turn_into_grace_chord) {
		...
		if (prev_elem_infos && prev_elem_ties && next_elem_infos && next_elem_ties && next_duration.r_num > 0 && prev_duration.r_num > 0) {
            ...
            for (tieelem = prev_ties->l_head, pitchelem = prev_mc->l_head; tieelem && pitchelem; tieelem = tieelem->l_next, pitchelem = pitchelem->l_next) {
				long this_tie = get_tie_corresponding_to_mc(these_ties, these_mc, hatom_getdouble(&pitchelem->l_hatom), CONST_TOLERANCE_MC, true);
				double this_pitch_mc = hatom_getdouble(&pitchelem->l_hatom);
				long this_num_unisons = how_many_doubles_in_llll_first_level(these_mc, this_pitch_mc);
//				long next_num_unisons = how_many_doubles_in_llll_first_level(next_mc, this_pitch_mc);
				long prev_num_unisons = how_many_doubles_in_llll_first_level(prev_mc, this_pitch_mc);
                
				if (hatom_getlong(&tieelem->l_hatom) && // there was a tie
					this_num_unisons > 0 && // there's actually a "this_pitch_mc" pitch in these_mc
					this_num_unisons >= prev_num_unisons && // the number of current pitches in this chord is NOT smaller than the one in the previous chord
					(this_tie < 0 || this_tie == 1)) {
//					((x->regroup_delete == k_DELETE || force_delete) && (this_tie < 0 || this_tie == 1))) {

// WAS: BUT WHY?!?!?!?
/*				if (hatom_getlong(&tieelem->l_hatom) && // there was a tie
					(!(this_num_unisons > prev_num_unisons)) && // the number of current pitches in this chord is NOT greater than the one in the previous chord
					is_double_in_llll_first_level(these_mc, hatom_getdouble(&pitchelem->l_hatom)) && 
					(((x->regroup_delete == k_DELETE || force_delete) && (this_tie < 0 || this_tie == 1)) ||
					 ((x->regroup_delete != k_DELETE || force_delete) && merging_direction >= 0)) && // we don't have the current tie information OR the tie was there!
					(is_double_in_llll_first_level(next_mc, hatom_getdouble(&pitchelem->l_hatom)) || (merging_direction >= 0 && x->regroup_delete != k_DELETE && !force_delete))) {
*/

					// nothing to do, we keep the tie
				} else if (hatom_getlong(&tieelem->l_hatom)) {
		...
		if (x->regroup_delete != k_DELETE && !force_delete && allow_merging && this_duration.r_num > 0) {
			if (merging_direction <= 0) { // merged to left
				...
				if (x->regroup_delete == k_MERGE) {
					long i;
					t_llll *cloned_prev_mc = llll_clone(prev_mc);
					t_llll *which_indices = llll_get();
					for (i = 0, elem = these_mc->l_head, elem2 = these_ties->l_head; // elem3 = these_infos->l_head; 
						 elem; 
						...
						if (!is_double_in_llll_first_level_with_tolerance(cloned_prev_mc, this_double, CONST_TOLERANCE_MC)) {
							llll_appenddouble(prev_mc, this_double, 0, WHITENULL_llll);
							if (elem2)
								llll_appendhatom_clone(prev_ties, &elem2->l_hatom, 0, WHITENULL_llll);
							else
								llll_appendlong(prev_ties, 0, 0, WHITENULL_llll);
//							if (elem3)
								llll_appendlong(which_indices, i + 1, 0, WHITENULL_llll);
						}	 
					}
					merge_infos(prev_infos, these_infos, prev_is_a_pause, which_indices); // the chosen indices
					llll_free(cloned_prev_mc);
				...
				//			llll_to_char_array(prev_infos, debug1, 999);
				//			llll_to_char_array(prev_ties, debug2, 999);
				
				// did we turn a pause into a note?!? – BUT DO WE WANT THIS?!?!?!? I THINK WE DON't... that's why I comment it (dg 6nov2012)
//				if (this_duration.r_num > 0 && prev_duration.r_num < 0)
//					hatom_setrational(&prev_elem_rhythm->l_hatom, rat_long_prod(hatom_getrational(&prev_elem_rhythm->l_hatom), -1));
			...
			if (merging_direction >= 0) { // merged to right
				t_llllelem *elem, *elem2; //was: *elem3;
				char next_is_a_pause = (next_duration.r_num < 0) ? true : false;
				if (x->regroup_delete == k_MERGE) {
					long i;
					t_llll *cloned_next_mc = llll_clone(next_mc);
					t_llll *which_indices = llll_get();
					for (i = 0, elem = these_mc->l_head, elem2 = these_ties->l_head; // elem3 = these_infos->l_head; 
						 elem; 
						 i++, elem = elem->l_next, elem2 = elem2 ? elem2->l_next : NULL) { // elem3 = elem3 ? elem3->l_next : NULL) {
						double this_double = hatom_getdouble(&elem->l_hatom);
						if (!is_double_in_llll_first_level_with_tolerance(cloned_next_mc, this_double, CONST_TOLERANCE_MC)) {
							llll_appenddouble(next_mc, this_double, 0, WHITENULL_llll);
							if (elem2)
								llll_appendhatom_clone(next_ties, &elem2->l_hatom, 0, WHITENULL_llll);
							else
								llll_appendlong(next_ties, 0, 0, WHITENULL_llll);
							// was: if (elem3)
							llll_appendlong(which_indices, i + 1, 0, WHITENULL_llll);
						}	
					}
					merge_infos(next_infos, these_infos, next_is_a_pause, which_indices); // the chosen indices
					llll_free(cloned_next_mc);
					llll_free(which_indices);
				} else { // merge with repetitions
				...
				// did we turn a pause into a note?!? – BUT DO WE WANT THIS?!?!?!? I THINK WE DON't... that's why I comment it (dg 6nov2012)
//				if (this_duration.r_num > 0 && next_duration.r_num < 0)
//					hatom_setrational(&next_elem_rhythm->l_hatom, rat_long_prod(hatom_getrational(&next_elem_rhythm->l_hatom), -1));
...
// merge, when possible, the notes smaller than x->box_tie_percentage * minimum_unit
	...
	if ((*rhythm) && (*rhythm)->l_size > 0) {
		while ((*rhythm)->l_size > 1 &&  
			...
			destroy_rhythm_element(x, (*rhythm)->l_head, (*infos)->l_head, (*ties)->l_head, prev_elem_rhythm, prev_elem_infos, prev_elem_ties, 
		...
		while ((*rhythm)->l_size > 1  &&  
			...
			destroy_rhythm_element(x, (*rhythm)->l_tail, (*infos)->l_tail, (*ties)->l_tail, 
...
/*		else if	(x->regroup_delete == k_TURN_INTO_GRACE_CHORDS && this_dur.r_num == 0) {
		...
		if (delete_elem) {
			...
			destroy_rhythm_element(x, rhythm_elem, infos_elem, ties_elem, 
					...
					t_llllelem *next_elem_rhythm, t_llllelem *next_elem_infos, t_llllelem *next_elem_ties)
{
	...
	systhread_mutex_lock(x->n_mutex);
	llll_clone_upon(box_durations, box_durations_wk);
	llll_clone_upon(box_infos, box_infos_wk);
	llll_clone_upon(box_ties, box_ties_wk);
	systhread_mutex_unlock(x->n_mutex);
