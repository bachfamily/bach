...
void C74_EXPORT ext_main(void *moduleRef)
{
    ...
    CLASS_ATTR_LONG(c,"slopemapping",0, t_integrate, slope_mapping_type);
		...
		if (ll) {
			...
			if (x->mode == 0) { // automatic mode
				...
				for (i = 0; i < x->order; i++) {
					...
					t_llll *res = integrate_bpf(ll, 0, 0, num_samples, this_origin, true, (e_slope_mapping)x->slope_mapping_type);
					llll_free(ll);
					ll = res;
				}
			} else {
				...
				for (this_order = 1; this_order < order; this_order ++) {
					...
					t_llll *res = integrate_bpf_with_explicit_sampling(ll, sampling_x, this_origin, (e_slope_mapping)x->slope_mapping_type);
					llll_free(ll);
					ll = res;
				}
