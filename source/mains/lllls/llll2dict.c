			...
			if (!subll->l_head) {
				...
				if (key) {
                    if (!subll->l_head->l_next) {
                        ...
                        if (subll->l_depth > 1 && subll->l_size > 1 && subll->l_head->l_next &&
                            hatom_gettype(&subll->l_head->l_next->l_hatom) == H_LLLL && 
                            hatom_getllll(&subll->l_head->l_next->l_hatom)->l_head &&
                            hatom_gettype(&hatom_getllll(&subll->l_head->l_next->l_hatom)->l_head->l_hatom) != H_SYM) {
                            ...
                            for (t_llllelem *el = subll->l_head->l_next; el; el = el->l_next, i++) {
                                if (hatom_gettype(&el->l_hatom) != H_LLLL) {
                                    object_error(x, "Non-llll value expected for llll-key %s - could not be converted.", key->s_name);
                                } else {
