...
bach also contains some lexers and parsers written in Antlr4. As with the Flex/Bison parsers, the bach repository contains both the Antlr4 grammar files (.g4), with the makefiles invoking Antlr4, and the C++ code files they produce. Install Antlr4 on both Mac and Windows should not be problematic, so the preferred procedure is to do so and keep everything as it is. Should you want to avoid installing Antlr4, you can modify the makefile and the relevant build targets.
